...
import {stemFor,polyphony,stemsFor} from '../packages/audio/mix';
...
const input=process.argv[2],output=process.argv[3],variant='DRAFT';
if(!input||!output)throw Error('Usage: render_collision_riffs.ts bundle.json NEW_OUTPUT_DIR');
...
if(!bundle.sound_design)throw Error('Riff design required');
for(const [file,expected] of Object.entries(bundle.sound_design.source_hashes))if(sha(fs.readFileSync(file))!==expected)throw Error('Stale source: '+file);
const events:ScoreEvent[]=bundle.events;
...
polyphony(events);
const frames=Math.round(duration*rate),gain=10**(bundle.sound_design.default_gain_db/20);
const stems=stemsFor(events);
...
for(const samples of Object.values(all))for(let i=frames-480;i<frames;i++)samples[i]*=(frames-1-i)/479;
...
const report={version:'performance-export-3',status:'DRAFT_NOT_APPROVED',approval:null,
 renderer:'collision-riff-node-mix-1',browser_measured:false,physical_output_measured:false,
 render_scope:'Existing voice() buffers mixed at fixed -9dB with a shared 10 ms final fade in Node; no live AudioContext, scheduler, output device or human audition',
 ...
 selection:bundle.selection,sound_design:bundle.sound_design,sound_design_bytes:bundle.sound_design_bytes,sound_design_sha256:bundle.sound_design_sha256,sound_design_event_bytes:bundle.sound_design_event_bytes,source_events_bytes:bundle.source_events_bytes,stem_layout:stems,final_fade_s:.01,
 ...
 mix_automation:[{scene_s:0,gain_db:bundle.sound_design.default_gain_db,muted:false,lanes:{soundtrack:true,foley:true}}],
 ...
 source_hashes:Object.fromEntries(['packages/audio/engine.ts','packages/audio/mix.ts','packages/audio/model.ts','tools/render_collision_riffs.ts','packages/audio/riff-voices.ts','modules/arranger/collision_riffs.py'].map(p=>[p,sha(fs.readFileSync(p))]))};
