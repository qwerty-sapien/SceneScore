...
const codeHashes=Object.fromEntries(await Promise.all(['tools/scenescore-server.ts','tools/scenescore-render.ts','apps/web/src/scenescore/model.ts','packages/audio/keyboard-scenes.ts','packages/audio/keyboard-score.ts','packages/audio/keyboard-voice.ts','packages/audio/engine.ts','packages/audio/riff-voices.ts'].map(async name=>[name,sha(await fs.readFile(path.join(root,name)))])));
const jsonFile=async(file:string,value:unknown)=>fs.writeFile(file,JSON.stringify(value,null,2));
type Job={id:string;name:string;stage:string;error?:string;duration?:number;trim?:{start_s:number;duration_s:number};frames?:{file:string;at:number;sha256:string}[];analysis?:ReturnType<typeof parseAnalysis>;revision?:string;result?:any;source_sha256?:string;api?:any;controller?:AbortController};
...
function command(job:Job,exe:string,args:string[],timeout=120000):Promise<string>{
 return new Promise((resolve,reject)=>{
  ...
  child.on('close',async code=>{
   ...
   await fs.writeFile(path.join(base,job.id,`process-${child.pid}.log`),output).catch(()=>{});
...
async function analyze(job:Job){
 ...
 await jsonFile(path.join(dir,'api-meta.json'),job.api);
...
async function render(job:Job,markers:unknown,settings:any){
 ...
 if((await fs.readdir(path.join(base,job.id),{withFileTypes:true})).filter(e=>e.isDirectory()).length>=20)throw Error('This session has 20 renders. Archive old renders before continuing.');
 const revision=randomUUID(),dir=path.join(base,job.id,revision);await fs.mkdir(dir);
 await stage(job,'Composing music');
 await jsonFile(path.join(dir,'request.json'),{duration:job.duration,markers,settings,source_sha256:job.source_sha256,source_trim:job.trim??null,analysis_mode:job.api?.source_mode??'manual_plan',api_response_id:job.api?.response_id??null,version,code_sha256:codeHashes,node_version:process.version});
...
async function processVideo(job:Job){
 ...
 const decoded=await command(job,ffmpeg,['-hide_banner','-nostdin','-n','-threads','2','-i',path.join(dir,'source'),'-map','0:v:0','-an','-ss',String(job.trim?.start_s??0),'-t',String(job.trim?.duration_s??30),'-vf',"scale=w='min(1280,iw)':h='min(720,ih)':force_original_aspect_ratio=decrease,scale=trunc(iw/2)*2:trunc(ih/2)*2,setsar=1",'-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p','-threads','2','-movflags','+faststart','-progress','pipe:1',path.join(dir,'preview.mp4')]);
 ...
 if(declared!==null&&!job.trim)durationCheck(declared);
...
function launch(job:Job,work:()=>Promise<void>){active=job;job.stage='Queued';job.controller=new AbortController();void work().catch(async error=>{job.error=job.controller?.signal.aborted?'Cancelled. Upload again or rebuild an existing analysis.':String(error.message||error);await stage(job,'Failed');}).finally(()=>{job.controller=undefined;if(active===job)active=undefined;});}
...
const vite=await createViteServer({configFile:false,root:path.join(root,'apps/web'),publicDir:false,server:{host:'127.0.0.1',middlewareMode:true,hmr:{host:'127.0.0.1',port:port+100},fs:{allow:[root],deny:['**/.env','**/.env.*','**/*.{crt,pem}','**/.git/**','**/artifacts/scenescore/**']}},appType:'mpa'});
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='GET'&&parts[2]==='sessions'){
   const sessions=await Promise.all((await fs.readdir(base)).filter(id=>/^[a-f0-9-]{36}$/.test(id)).map(async id=>{try{const j=await getJob(id);return {id:j.id,name:j.name,stage:j.stage,ready:Boolean(j.result),duration:j.duration};}catch{return null;}}));
   send(res,200,sessions.filter(Boolean));return;
  }
  if(req.method==='POST'&&['upload','example'].includes(parts[2])){
   ...
   try{
    ...
    let bytes:Buffer;
    if(parts[2]==='example'){const data=JSON.parse((await body(req,1024)).toString());if(!['A','B'].includes(data.name))throw Error('Unknown example');job.name=`${data.name}.mov`;bytes=await fs.readFile(path.join(root,'..',job.name));if(data.name==='B'){job.trim={start_s:0,duration_s:29};job.name='B.mov · first 29 seconds';}}
  ...
  if(req.method==='POST'&&parts[3]==='manual'){
   if(active){send(res,409,{error:'A video is already processing.'});return;}
   durationCheck(job.duration!);if(job.analysis)throw Error('An analysis already exists; edit its events instead.');
   job.analysis={summary:'Manual arrangement. API analysis was unavailable; no detected events are claimed.',objects:[],limitations:['Manually placed events only.'],markers:[]};job.api={source_mode:'manual_plan'};job.error=undefined;
   launch(job,()=>render(job,[],DEFAULT_SETTINGS));send(res,202,publicJob(job));return;
  }
  if(req.method==='POST'&&parts[3]==='recover'){
   if(active){send(res,409,{error:'A video is already processing.'});return;}
   const data=JSON.parse(await fs.readFile(path.join(base,job.id,'api-response.json'),'utf8'));
   if(data.status!=='completed')throw Error('No complete saved API response is available.');
   const text=data.output.flatMap((o:any)=>o.content??[]).filter((c:any)=>c.type==='output_text').map((c:any)=>c.text).join('\n');
   job.analysis=parseAnalysis(JSON.parse(text),job.duration!);
   const meta=await fs.readFile(path.join(base,job.id,'api-meta.json'),'utf8').then(JSON.parse).catch(()=>({requested_model:model,returned_model:data.model,response_id:data.id,usage:data.usage,sampling_fps:5,audio_analyzed:false,elapsed_ms:null}));
   job.api={...meta,source_mode:'cached_gpt',recovered_at:new Date().toISOString(),recovery_note:'Reparsed the saved response locally; no new API request.'};job.error=undefined;
   await jsonFile(path.join(base,job.id,'analysis.json'),{...job.analysis,api:job.api});
   launch(job,()=>render(job,job.analysis!.markers,DEFAULT_SETTINGS));send(res,202,publicJob(job));return;
  }
