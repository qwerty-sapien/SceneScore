...
export async function renderOffline(events:ScoreEvent[],duration:number,gainDb=-18,lanes:Lanes={soundtrack:true,foley:true},rate=48000):Promise<AudioBuffer>{
 if(events.length>5000||duration>120||duration<=0)throw Error('Offline render budget');
 const ctx=new OfflineAudioContext(2,Math.ceil(duration*rate),rate),gain=ctx.createGain();gain.gain.value=10**(gainDb/20);gain.connect(ctx.destination);
 for(const e of events){if(!(e.event_type==='foley'?lanes.foley:lanes.soundtrack)||e.resolved_time_s>=duration)continue;const node=ctx.createBufferSource();node.buffer=voice(ctx,e);node.connect(gain);node.start(e.resolved_time_s);}
...
export class Engine{
 ...
 async unlock(){if(!this.context){this.context=new AudioContext();this.master=this.context.createGain();this.master.connect(this.context.destination);this.context.onstatechange=()=>{if(this.context?.state==='suspended'&&this.timeline.playing){this.pause();this.onSuspend();}};}await this.context.resume();this.setGain(this.gainDb);this.prepare();}
 prepare(){if(!this.context)return;let frames=0;const needed=new Set<string>();for(const e of this.timeline.events){const k=this.key(e);needed.add(k);if(!this.cache.has(k))this.cache.set(k,voice(this.context,e));frames+=this.cache.get(k)!.length;if(frames*4>128*1024*1024)throw Error('Audio preparation memory budget');}for(const k of this.cache.keys())if(!needed.has(k))this.cache.delete(k);}
 ...
 async play(){await this.unlock();if(this.timeline.position>=this.timeline.bundle.scene.duration_s)this.timeline.reset(0,true);this.anchorScene=this.timeline.position;this.anchorAudio=this.context!.currentTime+.06;this.timeline.playing=true;this.fill();if(this.timer)clearInterval(this.timer);this.timer=setInterval(()=>this.fill(),25);}
 fill(){if(!this.context||!this.master||!this.timeline.playing)return;const pos=this.now();this.timeline.advance(pos);if(pos>=this.timeline.bundle.scene.duration_s){this.pause();this.onTick();return;}const now=this.context.currentTime,horizon=now+.15;
  this.nodes=this.nodes.filter(n=>n.end>now);
  for(const e of this.timeline.events){if(this.scheduled.has(e.id))continue;const start=this.anchorAudio+e.resolved_time_s-this.anchorScene,end=start+e.duration_s;if(end<=now){this.scheduled.add(e.id);continue;}if(start>horizon)continue;this.scheduled.add(e.id);if(!(e.event_type==='foley'?this.lanes.foley:this.lanes.soundtrack))continue;
   const source=this.context.createBufferSource(),gain=this.context.createGain();source.buffer=this.cache.get(this.key(e))!;source.connect(gain);gain.connect(this.master);const at=Math.max(now+.002,start),offset=Math.max(0,at-start);if(offset>=e.duration_s)continue;gain.gain.setValueAtTime(0,at);gain.gain.linearRampToValueAtTime(1,at+.003);source.start(at,offset);source.stop(end+.001);source.onended=()=>{source.disconnect();gain.disconnect();};this.nodes.push({source,gain,event:e,end});this.metrics.push({scene_s:e.resolved_time_s,audio_s:at,event_id:e.id,generation:this.timeline.generation});
  }this.onTick();
 }
 ...
 seek(at:number){this.clear();this.timeline.reset(at,true);this.prepare();this.onTick();}
 setGain(db:number){if(!Number.isFinite(db)||db< -36||db>0)throw Error('Gain range');this.gainDb=db;this.master?.gain.setTargetAtTime(this.mute?0:10**(db/20),this.context!.currentTime,.008);}
 setMute(mute:boolean){this.mute=mute;this.setGain(this.gainDb);}
 setLanes(lanes:Lanes){this.lanes=lanes;if(this.context)for(const n of this.nodes)n.gain.gain.setTargetAtTime((n.event.event_type==='foley'?lanes.foley:lanes.soundtrack)?1:0,this.context.currentTime,.008);}
 submit(action:ControlAction){const result=this.timeline.submit(action,this.context!.currentTime,this.now());if(result.status==='queued'){
   this.prepare();const at=this.anchorAudio+result.boundary_s!-this.anchorScene;
