...
import {verifyPianoMix,type PianoBinding} from './piano-binding';
...
export type Bundle=BlenderInputs&RiffBinding&PianoBinding&{version:'studio-bundle-1';id:string;title:string;variant:string;video:string;video_sha256:string;plan_bytes:string;plan_sha256:string;scene_hash:string;composition_hash:string;scene:SceneManifest;composition:CompositionSpec;groove:BrushGroove;states:ObjectState[];interactions:InteractionEvent[];events:ScoreEvent[];events_sha256:string;source:string;audition_status:string;music_vertical?:VerticalTransitionSidecar;
...
export async function verifyBundle(b:Bundle,video?:ArrayBuffer){
 ...
 await verifyPianoMix(b,p,sha,stable);
...
export function transition(events:ScoreEvent[],p:ArrangementPlan,c:CompositionSpec,tick:number,delta:number,tonic:number):ScoreEvent[]{
 ...
 for(const e of out){if(e.midi_pitch===null||e.resolved_time_s<at||e.resolved_time_s>=endBar)continue;
  if(e.lane_id==='bass'||/^(harmony|piano-comp)-[012]$/.test(e.lane_id)){const index=/^(harmony|piano-comp)-[012]$/.test(e.lane_id)?Number(e.lane_id.at(-1)):-1;const root=e.resolved_time_s<arrival?(to+7)%12:to;const interval=index<0?0:(e.resolved_time_s<arrival?[0,4,7]:intervals)[index];e.midi_pitch=bound(Math.floor(e.midi_pitch/12)*12+root+interval,p.register_min,p.register_max);e.phrasing=e.resolved_time_s<arrival?'new-dominant':'new-tonic-arrival';}}
 for(const e of [...out]){if(e.phrasing!=='new-dominant'||!(e.resolved_time_s<arrival&&arrival<e.resolved_time_s+e.duration_s))continue;const tail=copy(e);tail.id+=':arrival';tail.start_tick=tick+c.ppq;tail.duration_ticks=e.start_tick!+e.duration_ticks!-tail.start_tick;tail.resolved_time_s=arrival;tail.duration_s=e.resolved_time_s+e.duration_s-arrival;const interval=/^(harmony|piano-comp)-[012]$/.test(e.lane_id)?intervals[Number(e.lane_id.at(-1))]:0;tail.midi_pitch=bound(Math.floor(e.midi_pitch!/12)*12+to+interval,p.register_min,p.register_max);tail.phrasing='new-tonic-arrival';e.duration_ticks=tick+c.ppq-e.start_tick!;e.duration_s=arrival-e.resolved_time_s;out.push(tail);}
