...
class Workspace:
    def __init__(self, data_root, *, sources=None, trainer=None, predictor=None, subscriber_timeout=5):
        ...
        self.connected = self.connecting = self.training = False
        ...
        self.record_index = self.record_sequence = 0
    ...
    def status(self):
        with self.lock:
            return {"version": VERSION, "connected": self.connected, "connecting": self.connecting,
                    "mode": self.mode, "recording": self.recorder is not None, "source": self.source,
                    "channels": [] if not self.metadata else [{"name": c["name"], "unit": c["unit"]} for c in self.metadata["channels"]],
                    "sample_rate_hz": None if not self.metadata else self.metadata["sample_rate_hz"],
                    "measured_rate_hz": self.measured_rate_hz, "sample_count": self.sample_count,
                    "last_device_s": self.last_device_s, "device_epoch": None if not self.metadata else self.metadata["clock_epoch"],
                    "quality": self.quality, "reason": self.reason, "current_session_id": self.current_session_id,
                    "sessions": copy.deepcopy(list(self.records.values())), "model": self.model_summary(), "training": self.training}

    def model_summary(self):
        if self.model is None:
            return None
        keys = ("id", "status", "positive_examples", "negative_examples", "source_mode", "held_out", "model_kind", "control_authority")
    ...
    def connect(self, body):
        ...
        with self.lock:
            if self.worker is not None or self.connecting:
        ...
        try:
            ...
            metadata = {"kind": "AcquisitionMetadata", "schema_version": "0.1", "id": "training-metadata-" + secrets.token_hex(8),
                        "session_id": "preview-" + secrets.token_hex(8), "device_model": device_model,
                        "transport": "fixture" if mode == "synthetic" else "lsl", "sample_rate_hz": d["sample_rate_hz"],
                        "channels": [{**c, "enabled": True} for c in d["channels"]],
                        "clock_epoch": mode + "-" + secrets.token_hex(8), "hardware_verified": mode == "real_device", "raw_storage": "local_only",
                        "provenance": {"source_mode": mode, "creator": VERSION, "tool_version": VERSION,
                                       "config_hash": hashlib.sha256(encoded(d)).hexdigest(), "input_hashes": [], "seed": 1 if mode == "synthetic" else None}}
            with self.lock:
                ...
                self.last_subscriber = time.monotonic()
                self.stop_event = threading.Event()
                self.worker = threading.Thread(target=self._run, args=(source, generation, self.stop_event), name="muse-training-source", daemon=False)
                self.worker.start()
    ...
    def consume(self, rows, times, host_s):
        ...
        with self.lock:
            ...
            for row, timestamp in zip(rows, times):
                if len(row) != count or any(not math.isfinite(x) for x in row) or not math.isfinite(timestamp) or timestamp < 0:
            ...
            for row, timestamp in zip(rows, times):
                ...
                current.append(list(row)); current_times.append(timestamp); prev = timestamp
            if current:
                groups.append((current, current_times))
            ...
            self.reason = "Synthetic rehearsal; no hardware claim" if self.mode == "synthetic" else "Live samples; contact quality unverified; no music authority"
            if len(self.rows) >= 256:
                tail = list(self.rows)[-256:]
                self.measured_rate_hz = None if any(x[2] for x in tail[1:]) else (len(tail) - 1) / (tail[-1][0] - tail[0][0])
            if self.recorder is not None:
                for data, timestamps in groups:
                    gap = 0 if self.record_previous is None else max(0, round((timestamps[0] - self.record_previous) * rate) - 1)
                    metadata = self.recorder.metadata
                    chunk = {"kind": "EEGChunk", "schema_version": "0.1", "id": metadata["session_id"] + f"-{self.record_sequence}",
                             "session_id": metadata["session_id"], "provenance": metadata["provenance"], "sequence": self.record_sequence,
                             "sample_start_index": self.record_index + gap, "sample_rate_hz": rate, "channels": metadata["channels"],
                             "device_times_s": timestamps, "device_epoch": metadata["clock_epoch"],
                             "host_receipt": {"seconds": host_s, "clock": "host_monotonic", "epoch": self.host_epoch},
                             "samples": data, "dropped_samples_before": gap,
                             "quality": {"state": self.quality, "reason": None if self.mode == "synthetic" else "contact_quality_unverified"}, "imu": None}
    ...
    def record_start(self, body):
        ...
        if role not in {"train", "development", "final_test"} or not isinstance(seconds, (float, int)) or not math.isfinite(seconds) or not 10 <= seconds <= 600:
        ...
        with self.lock:
            if not self.connected or self.last_device_s is None or self.recorder is not None or self.training:
                raise ValueError("connected_samples_and_idle_recorder_required")
            if len(self.records) >= 64:
                raise ValueError("session_capacity_exceeded")
            if any(s["participant_id"] == participant and s["refit_id"] == refit and s["role"] != role for s in self.records.values()):
                raise ValueError("refit_split_leakage")
            sid = "session-" + secrets.token_hex(10)
            meta = copy.deepcopy(self.metadata); meta["session_id"] = sid; meta["id"] = "metadata-" + sid
            self.recorder = Recorder(self.root / sid, meta, explicitly_started=True)
            self.current_session_id = sid
            self.record_index = self.record_sequence = 0
            self.record_previous = None
            self.record_deadline = time.monotonic() + seconds
            self.records[sid] = {"id": sid, "participant_id": participant, "refit_id": refit, "role": role,
                                 "source_mode": self.mode, "samples": 0, "duration_s": 0, "marker_count": 0, "reviewed_count": 0,
                                 "status": "recording", "start_s": None, "end_s": None}
            atomic(self.root / sid / "session.json", self.records[sid])
            atomic(self.root / sid / "consent.json", {"statement": statement, "recorded_at_utc": datetime.now(timezone.utc).isoformat(),
                   "local_only": True, "publication_consent": False, "seconds_limit": seconds, "device_model_attestation": meta["device_model"],
                   "source_descriptor": self.source, "hardware_verification_scope": "Operator named device and selected full descriptor; contact quality remains unverified"})
        return self.status()
    ...
    def _stop_record(self, reason):
        ...
        for marker in self.markers(sid):
            if marker["end_s"] is None:
                append(self.root / sid / "markers.jsonl", {**marker, "end_s": self.last_device_s,
                       "closed_reason": reason, "host_end_s": time.monotonic(), "revision": marker.get("revision", 0) + 1})
    ...
    def marker(self, body):
        ...
        if kind not in {"down", "up", "cue"} or class_name not in CLASSES or not isinstance(client, (int, float)) or not math.isfinite(client):
        ...
        with self.lock:
            ...
            if kind == "up":
                ...
                if value["end_s"] is not None:
                    return value
                value = {**value, "end_s": self.last_device_s, "browser_end_ms": client, "host_end_s": now, "revision": 1}
            else:
                value = {"id": marker_id, "class_name": class_name, "start_s": self.last_device_s,
                         "end_s": self.last_device_s + 2 if kind == "cue" else None,
                         "source": "guided_cue" if kind == "cue" else "keyboard_intent",
                         "review_required": True, "browser_start_ms": client, "host_start_s": now,
                         "device_epoch": self.metadata["clock_epoch"], "source_anchor_s": self.last_device_s,
                         "anchor_host_receipt_s": self.last_host_s, "anchor_age_s": now - self.last_host_s,
                         "clock_mapping_uncertainty_s": None, "is_ground_truth": False,
                         "timing_note": "Latest source-batch anchor; no calibrated browser-to-device clock mapping", "revision": 0}
    ...
    def review_get(self, sid):
        with self.lock:
            path = self.path(sid)
            return {"session": copy.deepcopy(self.records[sid]), "markers": self.markers(sid), "reviews": read_lines(path / "reviews.jsonl")}
    ...
    def review(self, body):
        ...
        if any(not isinstance(x, (int, float)) or not math.isfinite(x) for x in (start, end)) or not 0 <= start < end or end - start > 10:
        ...
        with self.lock:
            path = self.path(sid); record = self.records[sid]
            ...
            value = {"id": "review-" + secrets.token_hex(8), "marker_id": marker_id, "start_s": start, "end_s": end,
                     "class_name": body["class_name"], "certainty": body["certainty"], "reviewer": reviewer,
                     "notes": notes, "source": "human_adjusted_raw_trace_review", "device_epoch": manifest(path)["metadata"]["clock_epoch"],
    ...
    def trace(self):
        with self.lock:
            ...
            return self._trace(data, self.metadata, self.source)

    @staticmethod
    def _trace(data, metadata, source):
        if metadata is None:
            return {"channels": [], "times_s": [], "samples": [], "device_epoch": None, "source_mode": None, "quality": "unverified", "prediction": None, "gaps": []}
        stride = max(1, math.ceil(len(data) / 2048)); shown = data[::stride]
        indices = source["frontal_indices"]
        return {"channels": [{"name": metadata["channels"][i]["name"], "unit": metadata["channels"][i]["unit"]} for i in indices],
                "times_s": [r[0] for r in shown], "samples": [[r[1][i] for i in indices] for r in shown],
                "device_epoch": metadata["clock_epoch"], "source_mode": metadata["provenance"]["source_mode"],
                "quality": "good" if metadata["provenance"]["source_mode"] == "synthetic" else "unverified",
                "prediction": None, "gaps": [r[0] for r in data if r[2]], "decimation_stride": stride}
    ...
    def raw(self, sid):
        ...
        return {"metadata": manifest(path)["metadata"], "chunks": list(replay(path)), "markers": self.markers(sid),
                "reviews": read_lines(path / "reviews.jsonl"), "consent": json.loads((path / "consent.json").read_text())}
    ...
    def segment(self, sid, start, end):
        ...
        raw = self.raw(sid); meta = raw["metadata"]; d = raw["consent"]["source_descriptor"]
        data = [(t, r, bool(c["dropped_samples_before"] and i == 0)) for c in raw["chunks"] for i, (t, r) in enumerate(zip(c["device_times_s"], c["samples"])) if start <= t <= end]
        if not data:
            raise ValueError("segment_outside_recorded_data")
        result = self._trace(data, meta, d); result["reviews"] = raw["reviews"]
        return result
    ...
    def train(self):
        ...
        try:
            examples, contract = [], None
            for session in sessions:
                if session["status"] != "closed" or session["role"] == "final_test":
                    continue
                raw = self.raw(session["id"]); meta = raw["metadata"]; indices = raw["consent"]["source_descriptor"]["frontal_indices"]
                    ...
                    for c in raw["chunks"]:
                        ...
                        for t, row in zip(c["device_times_s"], c["samples"]):
                            if review["end_s"] - 2 - 1 / meta["sample_rate_hz"] <= t <= review["end_s"]:
                                times.append(t); rows.append([row[i] for i in indices])
                    if len(times) < 2 or times[0] > review["end_s"] - 2 + 1 / meta["sample_rate_hz"] or any(b - a > 1.5 / meta["sample_rate_hz"] for a, b in zip(times, times[1:])):
                        raise ValueError("two_seconds_contiguous_preceding_data_required")
                    examples.append({"session_id": session["id"], "participant_id": session["participant_id"], "refit_id": session["refit_id"],
                                     "role": session["role"], "source_mode": session["source_mode"], "review_id": review["id"], "certainty": "reviewed",
                                     "class_name": review["class_name"], "times_s": times, "samples": rows, "sample_rate_hz": meta["sample_rate_hz"],
                                     "start_s": review["start_s"], "end_s": review["end_s"]})
    ...
    def delete(self, body):
        ...
        with self.lock:
            ...
            self.model = None; self.model_revision += 1
