...
`baseline` and `compile_preview` produce review candidates, never human approval. `TransitionPreview(plan,ctx)` uses the same local transition compiler for audition before approval and always returns `preview_only`; it cannot construct an ApprovedSession or activate playback. `import_plan` validates without changing bytes. `exact_diff` lists changed fields. To approve, a human reviewer must review the exact plan/scene/score/policy and create a canonical `Approval` identifying reviewer/time/decision and `approved_payload_sha256`, with both exact input hashes. `ApprovedSession` requires that approval and exposes defensive copies of active plan, input context and events. A changed score, groove, trajectory, policy, brief or payload requires new approval. Tests construct clearly synthetic approvals; they do not approve delivered music.
